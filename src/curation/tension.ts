...
// silent.
//
// `positional` (U-3): a caller's position on a claim doc directly conflicts
// (R-1: assert × dispute) with another live position on the same doc.
// Logged automatically by vault_assert as a self-tension on the doc path
// (sourceA === sourceB === the doc), carrying `positionA`/`positionB` — the
// two conflicting positions' ids. Like inter-proposal, it is system-minted,
// never caller-loggable via vault_tension_log.
...
// `inter-proposal` and `positional` are deliberately absent: both are
// system-generated, not a taxonomy callers pick from — vault_tension_log's
// kind enum is this constant, so keeping them out keeps the kinds off the
...
// system-generated inter-proposal and positional kinds (produced only by
// stageActionWithConflictCheck and vault_assert respectively, always as a
// self-tension on the contested target).
...
export async function addTension(
  ...
  // An inter-proposal or positional tension is by construction a
  // self-tension on the contested target — enforce the shape so these
  // system-generated kinds cannot be minted between arbitrary, unrelated
  // documents.
  if (
    ...
    return err(
      new Error(
        `addTension: a '${input.kind}' tension is a self-tension — sourceA ` +
  ...
  if (
    ...
    (typeof input.positionA !== "string" ||
      input.positionA.trim().length === 0 ||
      typeof input.positionB !== "string" ||
      input.positionB.trim().length === 0)
  ) {
    return err(
      new Error("addTension: a 'positional' tension requires both 'positionA' and 'positionB'"),
    );
  }
...
export const STALE_TIER_LINT_COPY: Record<Exclude<TensionKind, "unspecified">, string> = {
  ...
  positional:
    "Unresolved positional tension — two principals still hold conflicting positions on " +
    "this doc. Decide explicitly: `accepted` if both stand, `invalid` if mis-logged.",
};
