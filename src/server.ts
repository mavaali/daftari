...
import { describeRatifyElicitation } from "./tools/staged-actions.js";
...
// ratify.
//
// Single-`id` calls only — a batch `ids` call always requires an explicit
// decision (elicitation shows one action's rationale; there is no form for
// "decide N actions at once"). createServer's CallTool handler only enters
// this path when `ids` is absent.
...
export function createServer(
  ...
  server.setRequestHandler("tools/list", async () => ({
    tools: exposed.map((t) => {
      const s = serializeToolDefinition(t);
      ...
      return {
        ...s,
        inputSchema: s.inputSchema as Tool["inputSchema"],
        outputSchema: s.outputSchema as Tool["outputSchema"],
      };
    }),
  }));
  ...
  server.setRequestHandler("tools/call", async (request, ctx) => {
    ...
    try {
      ...
      // (Decision 5) — single-`id` calls only; a batch `ids` call always
      // requires an explicit decision, so it falls straight through to the
      // tool handler's own validation. A direct call with the decision
      // inline never enters this branch — it keeps working for clients that
      // don't do elicitation, and on 2025-era connections the SDK's legacy
      // shim fulfils the form over the session.
      if (
        name === "vault_ratify" &&
        args.ids === undefined &&
