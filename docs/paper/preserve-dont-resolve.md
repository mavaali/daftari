...
recency axis. In the **control** — formal contract amendment chains, where a most-recent-
mentioning baseline is accuracy-sufficient by drafting convention (operative-amendment
idioms outnumber stale-restatement idioms >100:1) — accuracy does not separate
...
editor-provided ("rv per consensus #N"); post-cutoff items (2025–26) plus perturbation
...
currently-active such items across the three articles that maintain a consensus box with
...
value has no "tension" slot — modeled by the **forced** condition (the foil must pick a
