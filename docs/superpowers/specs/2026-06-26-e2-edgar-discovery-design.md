...
1. **`efts-search.ts`** — `searchFullText(query, opts)`: query EDGAR full-text search. **Endpoint verified live 2026-06-26:** `https://efts.sec.gov/LATEST/search-index?q=<phrase>&forms=<form>&ciks=<10-digit-cik>&from=<offset>` — `q`/`forms`/`ciks`/`from` all work via curl+UA; hits carry `_source.{ciks, root_forms, file_date}` and `_id` = `<accession>:<filename>`. Handle pagination and the **10k result-window cap** (proven: ~100 hits/page, `hits.total.value` caps at 10000). Returns normalized hits `{cik, accession, filename, formType, fileDate, description}`. Pure parsing of an EFTS JSON response is separated from the network call (curl+UA, injectable transport — same pattern as E1's `fetchFiling`) so parsing is hermetically testable against a recorded fixture.
...
4. **`reconstruct.ts`** — `reconstructChains(cik, docs): Seed[]`: group a CIK's amendment exhibits by `(agreementType, baseDate)` signature, order by ordinal, identify the base, emit E1-format `Seed`s. Pure (operates on already-fetched `{ref, text}` records).
   - **`SeedDoc.role` mapping** (follow E1's `chain-docs.test.ts` convention): base → `"master"` (or `"master-ar"` when the base is an Amended & Restated agreement), amendment with ordinal *N* → `"amendment-N"`. `order` = 0 for the base, then ordinal index.
   - **`Seed.unitType` sequencing:** `reconstruct.ts` sets it to a placeholder `"unknown"` (it hasn't parsed citations yet); **`scoreChain` produces the authoritative `unitType`** (Section / defined-term / mixed) from which `parseCitations` path fired, and the runner writes the score-derived value into the emitted seed + manifest. So `unitType` has one authoritative producer (`score.ts`), not two.
