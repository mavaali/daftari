...
`limit` = Stage-1 default (10). **Sweep `maxAdd` rather than picking points** (Issue A — and the recall arm is `$0`, so a sweep costs nothing and removes any arbitrary choice). Per question, retrieve **once** at a generous `limit` (≥ `limit + maxWindowDays`, capped at the vault size) and derive everything from the cached results:
- **Full coverage-candidate list (computed once):** the same-tag, in-window docs not in the top-`limit`, ordered exactly as the shipped `gatherCandidates` does (`created DESC, path ASC`) — but **uncapped**. For a given `maxAdd = m`, coverage-ON's set = top-`limit` ∪ first-`m` candidates. This faithfully reproduces the real `applyCoveragePass` selection at every `m` (the window is fixed by the top-`seedK` seeds, stable across `limit`).
- **Two recall curves over `m`:** `coverage-recall(m)` (date-window selection) vs `rankExtension-recall(m)` = recall of top-`(limit+m)` ranked (the matched-budget baseline, also a curve). The sweep runs `m ∈ [0 .. fullWindowSize]`; `m=0` is the plain top-`limit` baseline, the top end is the uncapped mechanism ceiling.
- **Mark the as-shipped point `m = 5`** (`DEFAULT_COVERAGE_OPTIONS.maxAdd`) on every curve — it carries the deployment claim, but it is one labeled point, not a privileged comparison.

The only fixed number is `padDays=7` (the shipped default), which sets the window width and hence the sweep's top end — the honest mechanism ceiling, not an arbitrary cap.

Map each returned `path` → day number. **Pin the embedding provider** and assert `vectorUsed` is identical for every retrieval (a flipped vector half would change ranking and invalidate the comparison). **Record realized doc counts** along each curve — coverage-ON's realized adds can be fewer than `m` (it only adds in-window candidates that exist), so the curves are compared at an equal-budget *ceiling*, and the report shows realized counts alongside.

### 3. Span-recall metric (cheap arm, the gate)
Per question: `recall = |returnedDays ∩ qa.relevantDays| / |qa.relevantDays|`. The deliverable is the **two recall curves over `maxAdd`** from §2 — `coverage-recall(m)` vs `rankExtension-recall(m)` — plus the `m=0` baseline (plain top-`limit`) and the marked as-shipped point (`m=5`). Aggregate split by single-day vs multi-day, and **condition recall on `|relevantDays|` length** (report the length distribution; the length-7 modal cohort is where the as-shipped `m=5` is cap-limited — the curve makes that visible rather than hidden).

**Also report, at each `m`, the relevant-vs-distractor split of the *added* docs** (how many additions fell in `relevantDays`). This is the key disambiguator for a later backfire: it separates "distractors swamped a real recall gain" from "the cap starved the recall gain so only distractors were added." Note the shipped `gatherCandidates` takes the **newest** in-window docs (`created DESC`, then `slice`), so for a span like `[8..14]` the early additions skew recent — a selection limitation visible as the *shape* of `coverage-recall(m)` (if it rises slowly then jumps, recency-ordering is delaying the older relevant days).

No LLM; deterministic; `$0`.

### 4. Gated LLM arm (only if the recall gate passes)
Reuse the oracle harness machinery (`/tmp/oracle-recall.mjs`: answerer `anthropic/claude-haiku-4.5`, grounded judge `openai/gpt-5.4-mini`, OpenRouter via `integrations/recall-bench/.env`, concurrency 6, retries, judge `max_tokens ≥ 16`). For each **multi-day** question, build the context from coverage-OFF days and from coverage-ON days, answer each, and judge `hallucination` (1=grounded, 0=hallucinated). **Run the LLM arm at the as-shipped `m=5`** (the product claim — what users actually get), and **additionally at the uncapped point only if** the recall curve shows the uncapped end recovers materially more relevant days than `m=5` (i.e. `m=5` is cap-starved) — that second run answers "does the recall lever pay off in hallucination when the cap isn't the bottleneck," and is skipped when `m=5` already captures the span (no extra spend for no question). Multi-day-only is the fair scope: on a single-day question every coverage-added doc is by construction a distractor, so including them would *guarantee* backfire and measure nothing about the recall lever — but the writeup must state explicitly that **the hallucination finding is scoped to multi-day questions** and does not generalize to the single-day majority. Multi-day-only also bounds cost to a fraction of the ~$400 full SP1 run; the gate means we only spend if recall improved.

### 5. Success / kill criteria (adjustable)
**Recall gate** (read off the curves, not a single point):
- *Proceed* → the `coverage-recall(m)` curve sits **above** `rankExtension-recall(m)` by ≥ ~5pp on the multi-day set across the sweep (judged where the cap doesn't bind — at/near the uncapped end — so a real selection advantage isn't masked by the as-shipped cap). Separately report whether the as-shipped `m=5` point clears the bar (the deployment claim).
- *Kill* → the coverage curve does not exceed rank-extension at any budget. Date-window selection adds nothing over just taking more next-ranked docs. Write the negative result; skip the LLM arm.
...
- **Disambiguating a backfire (required before attributing it):** a backfire has two distinct causes that look alike — (i) real recall gain swamped by undemoted distractors (the suppression story), vs (ii) the as-shipped `m=5` cap starved the recall gain so coverage added mostly distractors and little signal (the cap story). Use the added-docs relevant-vs-distractor split (§3) and the **uncapped end of the recall curve** to tell them apart: if the uncapped point recovers the relevant span but the LLM arm still backfires there, it's (i); if recall barely moves even uncapped, the lever — not suppression — is the issue. Do not attribute the cause without this.
...
- **`maxAdd`=5 binds on the MODAL case, not the tail.** RB multi-day `relevantDays` are contiguous weekly spans — ~71% are exactly length 7 (>5), ~75% are ≥6. So coverage-ON recall at the as-shipped cap is mechanically limited for the majority of multi-day questions, and a weak/null result *at `m=5` alone* would be an artifact. Mitigation is built into the design (§2 `maxAdd` sweep → curves rather than points, §3 length-conditioned recall + added-doc relevant/distractor split, §5 gate read off the uncapped end of the curve), so the cap's cost is measured and visible, never silently absorbed. The `gatherCandidates` recency-skew (newest-first) is a real selection limitation of the shipped feature, visible as the shape of `coverage-recall(m)` — report it.
